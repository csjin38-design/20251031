...
## 작업 내용
- `temp.csv`를 파싱해 요약 통계, 수익률 지표, 상관관계 표를 계산했습니다.
- 롱온리 제약을 둔 평균-분산 최적화를 수행해 GMV·최대 샤프 포트폴리오와 효율적 투자선을 도출했습니다.
- 분석 결과와 SVG 시각화, 한국어 보고서(`report.md`), 정적 대시보드(`index.html`, `styles.css`, `script.js`)를 생성했습니다.
- Python 표준 라이브러리 기반 개발 서버(`server.py`)를 구현해 정적 자산을 로컬에서 제공합니다.
...
1. 의존성 설치 및 분석 재현
   ...
   python analysis.py
   ```
   위 명령을 실행하면 통계 표와 효율적 투자선 SVG가 `stdout`과 파일로 갱신됩니다.

2. 웹 대시보드 확인
   ```bash
   python server.py --host 0.0.0.0 --port 8000
   ```
   터미널에 표시된 주소(`http://<호스트>:8000`)로 접속하면 `index.html`을 비롯한 정적 자산을 확인할 수 있습니다. 실행을 마치려면 `Ctrl+C`를 누르세요.

### 로컬 미니 웹 서버 실행
정적 파일을 직접 열기 어려운 환경이라면 위 실행 방법 2의 명령을 사용하세요. 동일 네트워크 브라우저에서 `http://<호스트>:8000`으로 접속하면 `index.html`이 기본 페이지로 로드되며, `Ctrl+C`로 서버를 종료할 수 있습니다.
