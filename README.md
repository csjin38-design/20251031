...
### 로컬 미니 웹 서버 실행
정적 파일을 직접 열기 어려운 환경이라면 Python 표준 라이브러리만으로 동작하는 개발용 서버를 이용할 수 있습니다.

```bash
python server.py --host 0.0.0.0 --port 8000
```

위 명령을 실행하면 터미널에 접속 URL이 출력되며, 동일 네트워크의 브라우저에서 `http://<호스트>:8000`으로 접속하면 `index.html`이 기본 페이지로 노출됩니다. 서버는 `Ctrl+C`로 종료할 수 있습니다.
